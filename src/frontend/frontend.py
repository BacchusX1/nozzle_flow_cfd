...
from backend.drawing import NozzleGeometry, PolynomialElement, LineElement, ArcElement, TemplateLoader
from backend.standard_values import DEFAULTS
from backend.meshing import AdvancedMeshGenerator
from backend.simulation import SimulationSetup, SU2Runner
from backend.postprocessing import ResultsProcessor, InteractivePostprocessorWidget
...
class NozzleDesignGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        # Set application-wide cursor to standard arrow
        ...
        self.apply_theme()
        # Ensure consistent cursor across all widgets
        self._apply_standard_cursor()
        
    def _apply_standard_cursor(self):
        """Apply standard arrow cursor to all widgets recursively."""
        from PySide6.QtGui import QCursor
        standard_cursor = QCursor(Qt.CursorShape.ArrowCursor)
        
        def set_cursor_recursive(widget):
            widget.setCursor(standard_cursor)
            for child in widget.findChildren(QWidget):
                child.unsetCursor()  # Clear any custom cursor
        
        set_cursor_recursive(self)
    ...
    def _compute_scale_factor(self) -> float:
        ...
        Scales appropriately for common display setups:
        - FHD (1920x1080): 1.0
        - QHD (2560x1440): 1.15
        - UHD/4K (3840x2160): 1.5
        - 8K (7680x4320): 2.0
        
        Can be overridden with NOZZLE_UI_SCALE environment variable for testing.
        """
        import os
        
        # Check for environment variable override
        env_scale = os.environ.get('NOZZLE_UI_SCALE')
        if env_scale:
            try:
                override = float(env_scale)
                if 0.5 <= override <= 3.0:
                    print(f"Using NOZZLE_UI_SCALE override: {override:.2f}")
                    return override
            except ValueError:
                pass
        ...
        # Resolution-based scale factor using width as primary reference
        # FHD (1920) = 1.0, QHD (2560) = 1.15, 4K (3840) = 1.5, 8K (7680) = 2.0
        if width_px >= 7680:  # 8K
            resolution_scale = 2.0
        elif width_px >= 3840:  # 4K/UHD
            resolution_scale = 1.5
        elif width_px >= 2560:  # QHD/1440p
            resolution_scale = 1.15
        elif width_px >= 1920:  # FHD
            resolution_scale = 1.0
        else:  # Smaller screens
            resolution_scale = 0.85
        
        # DPI scale consideration (96 DPI = standard)
        dpi_scale = logical_dpi / 96.0
        
        # Device pixel ratio adjustment for HiDPI displays
        # Only apply if system is using native resolution (ratio > 1)
        if device_pixel_ratio > 1.2:
            # Display is in HiDPI mode, Qt is already scaling
            # Use a gentler scale increase
            combined_scale = resolution_scale * 0.9
        else:
            # Apply combined scaling
            combined_scale = resolution_scale * min(dpi_scale, 1.3)
    ...
    def _scaled(self, base_value: int) -> int:
        """Scale a pixel value based on current scale factor."""
        scale = getattr(self, 'scale_factor', 1.0) or 1.0
        return int(round(base_value * scale))
    
    def _font_px(self, size_type: str = 'normal') -> int:
        """Get scaled font size in pixels.
        
        Args:
            size_type: 'small', 'normal', 'medium', 'large', or 'xlarge'
        """
        size_map = {
            'small': int(getattr(Theme, 'FONT_SIZE_NORMAL', 14)) - 3,
            'normal': int(getattr(Theme, 'FONT_SIZE_NORMAL', 14)),
            'medium': int(getattr(Theme, 'FONT_SIZE_MEDIUM', 16)),
            'large': int(getattr(Theme, 'FONT_SIZE_LARGE', 18)),
            'xlarge': int(getattr(Theme, 'FONT_SIZE_LARGE', 18)) + 4,
        }
        return size_map.get(size_type, size_map['normal'])
    
    def apply_theme(self):
        """Apply clean modern theme with all values scaled for current resolution."""
        ...
        # Helper for scaling - all dimensions will use this
        def s(val): return int(round(val * scale))
        ...
        self.setStyleSheet(f"""
            ...
            /* === CONSISTENT CURSOR THROUGHOUT APP === */
            * {{
                /* Override any custom cursors - use standard arrow everywhere */
            }}
            ...
            QGroupBox {{
                ...
                margin-top: {s(22)}px;
                padding: {s(24)}px {s(12)}px {s(12)}px {s(12)}px;
            }}
            QGroupBox::title {{
            ...
            QComboBox::down-arrow {{
                ...
                border-top: {s(7)}px solid {Theme.TEXT_PRIMARY};
                margin-right: {s(8)}px;
            }}
        ...
        for i, (num, name, tooltip) in enumerate(step_names):
            ...
            if i < len(step_names) - 1:
                ...
                arrow.setStyleSheet(f"""
                    QLabel {{
                        color: {Theme.TEXT_SECONDARY};
                        font-size: {self._font_px('xlarge')}px;
                        font-weight: 300;
                        background: transparent;
    ...
    def _create_workflow_step(self, num, name, tooltip, is_active=False):
        ...
        step = QWidget()
        step.setFixedHeight(self._scaled(60))
        ...
        step.setStyleSheet(f"""
            QWidget {{
                ...
                padding: {self._scaled(6)}px {self._scaled(16)}px;
            }}
        """)
        
        layout = QHBoxLayout(step)
        layout.setContentsMargins(self._scaled(16), self._scaled(8), self._scaled(20), self._scaled(8))
        layout.setSpacing(self._scaled(14))
        
        # Step number circle
        num_label = QLabel(num)
        circle_size = self._scaled(32)
        num_label.setFixedSize(circle_size, circle_size)
        num_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        num_label.setStyleSheet(f"""
            QLabel {{
                background: {num_bg};
                color: {Theme.TEXT_PRIMARY};
                font-size: {self._font_px('normal')}px;
                font-weight: 700;
        ...
        for i, step in enumerate(self.workflow_steps):
            ...
            step.setStyleSheet(f"""
                QWidget {{
                    ...
                    padding: {self._scaled(4)}px {self._scaled(12)}px;
                }}
            """)
            ...
            if num_label:
                circle_size = self._scaled(24)
                num_label.setStyleSheet(f"""
                    QLabel {{
                        background: {num_bg};
                        color: {Theme.TEXT_PRIMARY};
                        font-size: {self._font_px('small')}px;
                        font-weight: 700;
                        border-radius: {circle_size // 2}px;
                    }}
                """)
            ...
            if name_label:
                name_label.setStyleSheet(f"""
                    QLabel {{
                        color: {text_color};
                        font-size: {self._font_px('normal') - 1}px;
                        font-weight: 600;
                        background: transparent;
                    }}
                """)
    ...
    def create_header_section(self):
        ...
        layout.setContentsMargins(self._scaled(16), self._scaled(8), self._scaled(16), self._scaled(8))
        ...
        subtitle.setStyleSheet(f"""
            QLabel {{
                color: {Theme.TEXT_SECONDARY};
                font-size: {self._font_px('small') - 2}px;
                font-weight: 400;
                font-family: Arial, sans-serif;
                background: transparent;
                padding: 0px;
    ...
    def create_status_card(self):
        ...
        card.setStyleSheet(f"""
            QGroupBox {{
                background: {Theme.SURFACE_VARIANT};
                border: 1px solid {Theme.BORDER};
                border-radius: {self._scaled(12)}px;
                font-size: {self._font_px('normal')}px;
                font-weight: 600;
                padding: {self._scaled(16)}px {self._scaled(8)}px {self._scaled(8)}px {self._scaled(8)}px;
            }}
        """)
        ...
        for key, status in self.status_labels.items():
            ...
            container_layout.setSpacing(self._scaled(8))
            ...
            status.setStyleSheet(f"""
                QLabel {{
                    ...
                    font-size: {self._font_px('small')}px;
                    font-weight: 500;
                    padding: {self._scaled(8)}px {self._scaled(12)}px;
                    background: {Theme.SURFACE};
                    border-radius: {self._scaled(8)}px;
                    border-left: {self._scaled(4)}px solid {Theme.WARNING};
                    min-height: {self._scaled(16)}px;
    ...
    def create_parameters_card(self):
        ...
        info_label.setStyleSheet(f"""
            QLabel {{
                ...
                font-size: {self._font_px('normal')}px;
                font-weight: 600;
                margin: {self._scaled(8)}px 0px {self._scaled(4)}px 0px;
            }}
        """)
    ...
    def update_modern_status(self, stage: str, completed: bool, progress: int = 0, message: str = ""):
        ...
        status_label.setStyleSheet(f"""
            QLabel {{
                ...
                font-size: {self._font_px('normal') - 1}px;
                font-weight: 500;
                padding: {self._scaled(12)}px {self._scaled(16)}px;
                background: {Theme.SURFACE};
                border-radius: {self._scaled(8)}px;
                border-left: {self._scaled(4)}px solid {border_color};
                min-height: {self._scaled(16)}px;
            }}
        """)
